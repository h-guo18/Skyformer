...
class EVA(LocalAttention):
    def __init__(self,
        ...
        if self.linformer:
            self.linformer_dim = 128
            
        #     # EF_proj = torch.zeros((2,input_size, self.linformer_dim))
        #     # torch.nn.init.normal_(EF_proj, mean=0.0, std=1/self.linformer_dim)
        #     # self.E = nn.Parameter(torch.Tensor(kwargs["num_heads"], self.linformer_dim, seq_len)).to("cuda:0")
        #     self.E_proj = nn.Linear(input_size, self.linformer_dim,bias=False)
        #     self.F_proj = nn.Linear(input_size, self.linformer_dim,bias=False)
        ...
        if self.attn_2d:
            ...
            if self.window_size > 0:
                if key_padding_mask is None:
                    ...
                    x, key_padding_mask = pad_to_multiple(x, 64, dim=-2, create_mask=True)
                else:
    ...
    def forward(self, q,k,v, key_padding_mask = None):
        ...
        # print("processd x",x.shape)
        # N = np.prod(seq_shape)
        # q, k, v = self.proj_and_split_heads(x)
        # print("q shape",q.shape)
        B,_,seq_shape,_ = q.shape
        orig_n = seq_shape
        N = seq_shape
        ...
        window_size_wq=int(self.window_size*(N/self.linformer_dim)) if self.linformer else self.window_size
        w_q = self.window_partition(q, seq_shape, window_size=window_size_wq,ext_window_size=0)
        ...
        if self.attn_2d:
            ...
            if self.linformer: assert N % self.num_landmarks == 0 and self.linformer_dim % self.num_landmarks == 0
            rf_win_size_q = int(N // self.num_landmarks)
            rf_win_size_kv = int(self.linformer_dim // self.num_landmarks) if self.linformer else rf_win_size_q
        # [b, h, c, j, d]
        rf_w_q = self.window_partition(q, seq_shape, window_size=rf_win_size_q, ext_window_size=self.ext_size)
        # [b, h, c, j, d]
        rf_w_k = self.window_partition(k, seq_shape, window_size=rf_win_size_kv, ext_window_size=self.ext_size)
        # [b, h, c, j, d]
        rf_w_v = self.window_partition(v, seq_shape, window_size=rf_win_size_kv, ext_window_size=self.ext_size)
        # compute local attention
        # [b, 1, c, j, 1]
        if not self.linformer:
            rf_w_mask = self.window_partition(
                key_padding_mask, 
                seq_shape, 
                window_size=rf_win_size_q,
